...
inline void cpu_alloc(std::vector<MyTensorInfo>& tensor_info) {
  for (auto& iter : tensor_info) {
    int64_t size = shape2size(iter.shape);
    void* cpu_data = malloc(size * sizeof(float));
    std::vector<float> cpu_gen_data = generate_data(size, 0);
    memcpy(cpu_data, cpu_gen_data.data(), size * sizeof(float));
    iter.data = cpu_data;
  }
  return;
}

inline void cpu_free(std::vector<MyTensorInfo>& tensor_info) {
  for (auto& iter : tensor_info) {
    free(iter.data);
  }
  return;
